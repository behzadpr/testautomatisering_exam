...
def step_no_favorite_books(context):
    """Ensure no books are favorited by navigating fresh (no selections made)"""
...
def step_see_default_message(context):
    """Verify a default/empty-state message is visible when there are no favorites"""
    assert context.favorites.is_mina_bocker_visible(), '"Mina böcker" view should be visible'
    assert context.favorites.has_default_message(), 'Default empty-state message should be visible'
...
def step_see_all_favorited_books(context):
    ...
    for i in range(context.favorites.get_book_count()):
        book_text = context.favorites.book_items.nth(i).text_content().strip()
        assert any(book_text in title for title in context.catalog_favorited_titles), (
            f'Book "{book_text}" should appear in favorited titles: {context.catalog_favorited_titles}'
        )
...
def step_verify_book_in_mina_bocker(context):
    ...
    context.favorites = MinaBockerPage(context.page)
    assert context.favorites.is_mina_bocker_visible(), '"Mina böcker" view should be visible'
    assert context.favorites.get_book_count() > 0, "Favorited book should be listed in Mina böcker"
    first_book_text = context.favorites.get_first_book().text_content().strip()
    assert first_book_text in context.catalog_current_book_title, (
        f'Expected "{context.catalog_current_book_title}" to appear in Mina böcker'
    )
